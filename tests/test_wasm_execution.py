...
def test_wasm_registry_excludes_desktop_and_external_tool_nodes() -> None:
    registry = create_wasm_node_registry()

    assert registry.get("create-cube") is not None
    assert registry.get("convert-mesh-file-format") is not None
    assert all(registry.get(definition.id) is definition for definition in IMAGE_TOOL_NODES)
    ...
    assert registry.get("mesh-sink") is None
    assert registry.get("load-meshio") is None
    assert registry.get("cosapp-workflow") is None


def test_wasm_execution_module_does_not_import_the_desktop_node_registry() -> None:
    source = (
        Path(__file__).parents[1] / "src" / "flowgraph" / "application" / "wasm_execution.py"
    ).read_text(encoding="utf-8")

    assert "from flowgraph.application.node_registry import create_node_registry" not in source
